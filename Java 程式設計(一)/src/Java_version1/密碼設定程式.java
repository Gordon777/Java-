...
public class 密碼設定程式 {
	public static void main(String args[]) {			
		...
		String name, key; //登錄使用者名稱, 鹽 
		int hash_pas; // 登錄密碼雜湊值 
		String passwd1, passwd2;
		int value1, value2; 
		System.out.printf("請輸入使用者名稱 =>"); 
		name = keyin.nextLine();
		System.out.printf("請輸入密碼明文 =>"); passwd1 = keyin.nextLine();
		value1 = 1 + (int)(Math.random()* 999); key = String.valueOf(value1);
		...
		System.out.printf("\n請輸入密碼 (驗證密碼是否正確) =>"); passwd1 = keyin.nextLine();
		System.out.printf("由系統中取出鹽 = %s \n", key); passwd2 = passwd1.concat(key);
		System.out.printf("輸入密碼與鹽結合後 = %s\n", passwd2); value2 = passwd2.hashCode();
